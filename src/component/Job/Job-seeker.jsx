...
import { FaMapMarkerAlt, FaBriefcase, FaCode } from "react-icons/fa"; // Using React Icons for a more professional look
import { FaMagnifyingGlass } from "react-icons/fa6"; // A different search icon for variety

// Sample job data with added 'skills' array
const sampleJobs = [

          //!  this is the pettern to insert the job
...
  //   description: "We are looking for a skilled Frontend Developer with React experience to join our team.ooooooooooooam",
...
  // }
  ...
  },
  {
    ...
    description: "Join our backend team working with Node.js and databases to build scalable APIs.",
  ...
  },
  {
    ...
    description: "Creative UI/UX Designer needed to craft intuitive interfaces and user experiences.",
  ...
  },
  {
    ...
    description: "Work on both the frontend and backend of our core application.",
...
export default function JobOpening() {
  ...
  useEffect(() => {
    // Simulate fetching job openings
    setIsLoading(true);
    setTimeout(() => {
      setJobs(sampleJobs);
  ...
  return (
    <div className="min-h-screen bg-[#F6F6F6] py-32 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        >
          Explore exciting career opportunities that match your skills and aspirations.
        </motion.p>
            ...
            {filteredJobs.map((job) => (
              ...
              >
                <div className="bg-[#B8E1DD] rounded-xl p-8 shadow-md hover:shadow-lg transition-all duration-300 border border-[#E0F0EF]">
                  <h2 className="text-2xl font-bold text-[#044A42] mb-2">{job.title}</h2>
                  <div className="flex items-center space-x-4 text-[#062925] mb-4">
                    <span className="flex items-center"><FaBriefcase className="mr-2 text-[#3A9188]" /> Full-time</span>
                    <span className="flex items-center"><FaMapMarkerAlt className="mr-2 text-[#3A9188]" /> {job.location}</span>
                  </div>
                  <p className="text-[#062925] mb-4">{job.description}</p>
                  
                  {/* Skills Section */}
                  <div className="mb-6">
                    <h4 className="text-lg font-semibold text-[#044A42] mb-2">Required Skills:</h4>
                    <div className="flex flex-wrap gap-2">
                      {job.skills.map((skill, skillIndex) => (
                        <span key={skillIndex} className="bg-[#3A9188] text-white text-sm font-medium px-4 py-1 rounded-full shadow-sm">
                          {skill}
                        </span>
                      ))}
                    </div>
